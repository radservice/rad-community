<%@page import="net.purwana.rads.apps.workflow.security.EnhancedWorkflowUserManager"%>
<%@ page import="net.purwana.rads.workflow.util.WorkflowUtil"%>
<%@ page import="net.purwana.rads.apps.app.service.AppUtil"%>
...
<c:if test="${isQuickEditEnabled || param.webConsole =='true'}">
    ...
    <c:if test="${isAdmin && (param.builderMode || !(isCustomAppAdmin && !empty param.webConsole))}">
        ...
        <div id="adminBar" class="adminBarInactive">
            <a id="appCenter" <c:if test="${empty param.webConsole}"> target="_blank"</c:if> title="<ui:msgEscHTML key='adminBar.label.appCenter'/>" href="${pageContext.request.contextPath}/home"><i class="fab fa-rads"></i></a>
            ...
            <c:if test="${!empty param.appId || !empty param.webConsole}">
                ...
                <div>
                    <a class="adminBarButton" title="CTRL-<c:out value="${key + 4}"/>: Support Online" href="http://rads.purwana.net/support-online" onclick="return AdminBar.showQuickOverlay('http://rads.purwana.net/support-online')"><i class="fas fa-donate"></i><span>Support Online</span></a>
                </div>
                </c:if>
            </c:if>
            </div>
...
<jsp:include page="/web/json/plugin/net.purwana.rads.apps.ext.ConsoleWebPlugin/service?spot=adminBar" flush="true" />             
